...
$mail = new PHPMailer;
	...
	$usermail = "noreply@pro-service-rostov.ru";
    $mail->setFrom($usermail, 'Обработчик почты pro-service-rostov.ru4');
    $mail->addAddress($sendto,'Ремонт с умом');
    $mail->addReplyTo($sendto,'Ремонт с умом');


     $mail->isHTML(true);   


		$siteName = "Ремонт с умом";
		$siteUrl = "pro-service-rostov.ru";
		...
		}else if(isset($_POST['name_c'])){
			$name = htmlspecialchars($_POST['name_c']);
			$name = trim($name);
            $name = strip_tags($name);
			$msg .= "<p><strong>Имя клиента:</strong> ".$name."</p>\r\n";
		}
		...
		}else if(isset($_POST['phone_c'])){
			$phone = htmlspecialchars($_POST['phone_c']);
			$phone = trim($phone);
            $phone = strip_tags($phone);
			$msg .= "<p><strong>Контактрый телефон:</strong> ".$phone."</p>\r\n";
		}
